...
class Consulation(models.Model):
    author = models.OneToOneField(ProfileEmployee, on_delete=models.CASCADE)
    created_date = models.DateTimeField(auto_now_add=True)
    mashroot_len = models.IntegerField(default=0)
    moadel = models.DecimalField(default=0, decimal_places=2, max_digits=4)
    arzyabi = models.CharField(
        max_length=20, choices=ARZYABI_CHOICES, default='عادی')
    ...
    model_term_ghabl = models.DecimalField(
        default=0, decimal_places=2, max_digits=4)
    ...
    slug = models.SlugField(
        max_length=200, allow_unicode=True, null=True, blank=True)
...
class AgentMarkaz(models.Model):
    markaz_moshavere = models.OneToOneField(
        MarakezMoshavere, on_delete=models.CASCADE)
    first_name = models.CharField(max_length=200)
    last_name = models.CharField(max_length=200)
